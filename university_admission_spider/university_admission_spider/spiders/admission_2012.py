...
import urllib
import scrapy
from university_admission_spider.items import UniversityAdmissionSpiderItem
...
class Score2011Spider(scrapy.spiders.Spider):
    ...
    params = {'page': 1, 'recomschtype': '普通本科', 'recomluqupici': '一批', 'scoreSign': 3, 'schoolSort': 4, 'recomyear': 2012, 'argprovince': '福建', 'argluqutype': '理科'}
    base_url = 'http://gkcx.eol.cn/soudaxue/queryProvinceScoreNum.html?%s'
    ...
    def parse(self, response):
        ...
        if len(tbody) > 0:
            ...
            for tr in tbody.xpath('.//tr'):
                if tr.re(r'queryProvinceScoreNumLeftad'):
                    pass
                else:
                    # print tr.extract()
                    item = UniversityAdmissionSpiderItem()
                    tds = tr.xpath('td/text()').extract()
                    item['university'] = tr.xpath('.//td//a/text()')[0].extract()
                    item['admission_count'] = tds[4]
                    item['year'] = tds[2]
